...
./copyInfo c1.cnf c1.cnf.end c2.keyInfo
...
../exp/geneInputFix key.keyInfo f$cnt.cnf.out.dat k$cnt.keyInfo
...
echo "copyCnf c1.cnf xort.cnf.end c1$cnt.cnf" 1>&2
...
../exp/geneBranch $3 c1.cnf.start c1$cnt.cnf.start c2$cnt.cnf.end brC1C1$cnt.cnf
...
../exp/geneBranch $3 c2.cnf.start c2$cnt.cnf.start brC1C1$cnt.cnf.end brC2C2$cnt.cnf

#MAX ID の確認
echo "End brC2C2$cnt.cnf.end $(cat brC2C2$cnt.cnf.end)"

echo "geneXORT c1.piCnfInfo c1.cnf.start c1$cnt.cnf.start brC2C2$cnt.cnf.end xxort1$cnt.cnf" 1>&2
../exp/geneXORT c1.piCnfInfo c1.cnf.start c1$cnt.cnf.start brC2C2$cnt.cnf.end xxort1$cnt.cnf
...
../exp/geneXORT c1.piCnfInfo c2.cnf.start c2$cnt.cnf.start xxort1$cnt.cnf.end xxort2$cnt.cnf
...
do
    ...
    echo "copyCnf XY$cnt.cnf xxort2$pre.cnf.end c1$cnt.cnf" 1>&2
    ../exp/copyCnf XY$cnt.cnf xxort2$pre.cnf.end c1$cnt.cnf
    ...
    # echo "cat c1$cnt.cnf XY1$cnt.cnf > c1$cnt.cnf" 1>&2
    # cat c1$cnt.b.cnf XY1$cnt.cnf > c1$cnt.cnf
...
# 鍵を特定する
./problem c1$cnt.cnf problem.cnf
...
cat c1*.cnf brC1C1*.cnf > decrypto.cnf
